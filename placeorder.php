...
<head>
<style>
#show{width:90%;height:auto;background:#fff;margin-left:70px;border-radius:4px;box-shadow: 3px 3px 3px #000;border-radius:4px;border:2px solid #400040;margin-top:20px;}
...
?>
	<div id="show">
	<h1 style="width:100%;height:40px;line-height:40px;background:#400040;color:#fff;text-align:center;margin-top:20px;border-radius:4px;text-shadow: 5px 5px 5px #000;box-shadow: 3px 3px 3px #000;">Детали заказа</h1>
				<table>
				<tr>
					<th>Номер</th>
					<th>Название</th>
				...
				?>
				<tr><td colspan="4" style="text-align:right;border-top:1px solid #400040;"><b style="color:red;font-size:25px;">Всего</b></td>
				<td style="border-top:1px solid #400040;border-bottom:1px solid #400040;">$ <b style="font-size:20px;"> <?php echo"".$b."";?></b></td></tr>
		<tr><td colspan="5">Способ оплаты: <?php echo"".$_GET['method']."";?></td></tr>	
<tr><td colspan="5"><form method="post"><input type="submit" name="place" value="Оформить заказ" style="padding:10px;border:1px solid #425298;color:#425298;background:#fff;border-radius:4px;"></form></td></tr>			
		</table>
	</div>
		<div id="show">
			
		<h1 style="width:95%;text-align:center;background:#400040;color:#fff;height:50px;line-height:50px;margin-left:30px;margin-top:10px;border-radius:4px;margin-bottom:10px;">Сведения о заказе </h1>
				<table>
				<tr>
					<th>Номер</th>
					<th>Название</th>
...
<?php
	...
	if(isset($_GET['upqty']))
	{
			...
			if($qty<=10)
			{
					...
					if($query->execute())
					{
						echo"<script>alert('Обновлено удачно')</script>";
						echo"<script>window.open('placeorder.php?method=cod','_self')</script>";
					}
					else
					{
						echo"<script>alert('Обновлено неудачно')</script>";
					}
			}
			else
			{
